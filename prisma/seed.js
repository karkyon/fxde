...
// Phase3 で argon2 本実装時に置き換える
const PLACEHOLDER_HASH = '$argon2id$v=19$m=65536,t=3,p=4$placeholder$placeholder_hash_replace_in_phase3';
async function main() {
    ...
    const adminSettings = await prisma.userSetting.upsert({
        ...
        create: {
            userId: admin.id,
            preset: client_1.Preset.standard,
            scoreThreshold: 75,
            riskProfile: {
                maxRiskPct: 1.0,
                maxDailyLossPct: 3.0,
    ...
    const freeSettings = await prisma.userSetting.upsert({
        ...
        create: {
            userId: demoFree.id,
            preset: client_1.Preset.conservative,
            scoreThreshold: 85,
            riskProfile: {
                maxRiskPct: 0.5,
                maxDailyLossPct: 1.5,
    ...
    const defaultTimeframes = {
        EURUSD: client_1.Timeframe.H4,
        USDJPY: client_1.Timeframe.H4,
        GBPUSD: client_1.Timeframe.H4,
        BTCUSD: client_1.Timeframe.D1,
    };
    ...
    const freeSymbol = await prisma.symbolSetting.upsert({
        ...
        create: {
            ...
            symbol: 'EURUSD',
            enabled: true,
            defaultTimeframe: client_1.Timeframe.H4,
        },
    });
